...
def getMongoUN(userType="casslabsadmin"):
    if userType == "ftb_field_tester" or userType == "ftb_engineer" or userType == "ftb_admin":
...
def accessMongoCollection(databaseName, collectionName, userType):
    if userType == "ftb_field_tester" or userType == "ftb_admin" or userType == "ftb_engineer":
        clientName = "mongodb+srv://"  + getMongoUN(userType) + ":" + getMongoPW(userType) + "@clientdata.3mjy7ss.mongodb.net/"
    elif userType == "casslabsadmin":
        clientName = "mongodb+srv://" + getMongoUN(userType) + ":" + getMongoPW(userType) + "@casslogger.atyux9l.mongodb.net/?retryWrites=true&w=majority"
...
def deleteMany(queryBy: str, id: str, myDatabase: str, myCollection: str, userType: str):
    ...
    if myCollection == col and myDatabase == db:
        ...
        for item in metadata:
            s3Success = s3.Object('fieldtestbuddy', item["filename"]).delete() #ehhhh ExpectedBucketOwner="616163632496"
...
def updateMany(queryBy: str, key: str, myDatabase: str, myCollection: str, updateField: str, updateVal: str, userType):
    collection = accessMongoCollection(myDatabase, myCollection, userType)
...
def ftbDbUploadBulk(filepath, postData, userType):
    ...
    files = list(filepath.glob('*.bin')) + list(filepath.glob('*.csv')) + list(filepath.glob('*.xls'))
...
def ftbDbUpload(filepath: str, filename: str, metadata: dict, userType):
    ...
    uuidFilename = postUUID + filename[-4:] # TODO: this assumes extension is three chars (bin, csv, xls, etc.)

    metadata.update({"_id":postUUID,
...
def createUserDict(un, pw, name, org, userType, email):
    ...
    userDict = dict(userName=un, password=pw, name=name, org=org, userType=userType, email=email)
    return userDict
...
def updateUser(queryBy: str, key: str, updateField: str, updateVal: str, userType):
    ...
    mongoSuccess =  collection.update_one({queryBy:key},{"$set":{updateField:updateVal}})
    return mongoSuccess

def updateUserPW(un, newPW, userType):
    newPW = ag.PasswordHasher().hash(newPW)
    return updateUser(queryBy="username",key=un, updateField="password", updateVal=newPW, userType=userType)